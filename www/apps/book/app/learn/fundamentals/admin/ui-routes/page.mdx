...
- `hidden`: an optional callback function to conditionally hide the sidebar item. Learn more in the [Conditionally Hide Sidebar Item](#conditionally-hide-sidebar-item) section.
...
### Conditionally Hide Sidebar Item

You can conditionally hide a sidebar item based on the current user by using the `hidden` property passed to `defineRouteConfig`. This is useful for implementing permission-based visibility.

For example, to hide a route from users who don't have an "admin" role in their metadata:

export const hiddenHighlights = [
  ["17", "hidden", "Conditionally hide the sidebar item."]
]

```tsx title="src/admin/routes/admin-only/page.tsx" highlights={hiddenHighlights}
import { defineRouteConfig } from "@medusajs/admin-sdk"
...
The `hidden` property accepts a callback function that:

- Receives the current authenticated user object (or `null` if not authenticated)
- Returns `true` to hide the item, `false` to show it

The user object includes `id`, `email`, `first_name`, `last_name`, and `metadata` properties. You can store custom roles or permissions in the `metadata` field.

<Note title="Important" type="warning">

The `hidden` property only hides the sidebar menu item. The route itself remains accessible via direct URL navigation. For true route protection, implement server-side middleware or API route guards.
